...
def svm_classifier(train, query, train_cols):
    ...
    print(clf.fit(train[train_cols], train['safebrowsing']))
    scores = cv.cross_val_score(clf, train[train_cols], train['safebrowsing'], cv=30)
...
def forest_classifier_gui(train, query, train_cols):
    ...
    print (rf.fit(train[train_cols], train['safebrowsing']))
...
def forest_classifier(train, query, train_cols):
    ...
    print (rf.fit(train[train_cols], train['safebrowsing']))
    scores = cv.cross_val_score(rf, train[train_cols], train['safebrowsing'], cv=30)
